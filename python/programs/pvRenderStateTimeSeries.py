...
def main():
    ...
    activeSource = sources.values()[0]
    pvsimple.SetActiveSource(activeSource)
    latestTime = activeSource.TimestepValues[-1]
    startTime = activeSource.TimestepValues[1]

    views = servermanager.GetRenderViews()

    for instant in activeSource.TimestepValues:
        for i,view in enumerate(views):
            view.ViewTime = instant
