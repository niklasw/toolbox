...
public class postRecording_alternate extends StarMacro {
  ...
  private void execute0() {
    ...
    List<Boundary> wallBoundaries = myP.getBoundaries(region_0,"wall");

    wallBoundaries.addAll(myP.getBoundaries(region_1,"pipe"));

    List<Boundary> filterInterfaces = myP.getBoundaries(region_2,"interface");


    Collection allBds = region_0.getBoundaryManager().getBoundaries();
    
    DirectBoundaryInterfaceBoundary directBoundaryInterfaceBoundary_0 = 
      ((DirectBoundaryInterfaceBoundary) region_0.getBoundaryManager().getBoundary("filter_interface_outer [a_upstream/b_filter 2]"));
    ...
    StreamPart streamPart_0 = 
      simulation_0.getPartManager().createStreamPart(new NeoObjectVector(simulation_0.getRegionManager().getRegions().toArray()), new NeoObjectVector( filterInterfaces.toArray()), primitiveFieldFunction_0, 10, 10, 2);
    ...
    scalarDisplayer_0.getScalarDisplayQuantity().setRange(new DoubleVector(new double[] {1000.0, 5000.0}));
    ...
    partDisplayer_0.getHiddenParts().addObjects(myP.getBoundaries(region_0,"wall_box_ext"));

    scalarDisplayer_0.getHiddenParts().addObjects(myP.getBoundaries(region_0,"wall_box_ext"));
