        ...
        for blockDef in blockDefs:
            ...
            for idx in indices:
                try:
                    ...
                    BM.error('Block node index out of range!')
                    print idx
            print nodes
            res   = map(int,blockDef[1].split())
            grad  = map(int,blockDef[2].split())
            if BM.OK:
...
class meshManipulator:
    ...
    def __init__(self):
        ...
        try:
            self.ob = Blender.Object.GetSelected()[0]
...
def gui():     
    ...
    Blender.Draw.Button('Read blocks',       35,  x(2),  y(8), w(2), h(1), 'Read block defs from file')
...
def button(evt):
    ...
    if evt == 8:
        ...
        if blockEngine.currentBlockIndex >= 0:
            ...
            nbrs = blockEngine.currentBlock.neighbours(blockEngine.blocks)
            for key in nbrs:
                if key == 'xmin' or key == 'xmax':
                    nbrs[key].res[1] = resY.val
                    nbrs[key].res[2] = resZ.val
                elif key == 'ymin' or key == 'ymax':
                    nbrs[key].res[0] = resX.val
                    nbrs[key].res[2] = resZ.val
    ...
    if evt == 35:
        blockList = BlockList()
        blockList.read(blockEngine)
        for block in blockList:
            print block
        blockEngine.insertBlockList(blockList)
